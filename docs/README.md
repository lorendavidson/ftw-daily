...
_Short answer:_ Demonstrate the features of Flex with a marketplace for renting venues. Because why
not.

_Long answer:_ Welcomely works as a template for a marketplace user interface. It can be used as a
marketplace client out of the box but is intended to be modified to match the requirements of each
marketplace built on top of the Flex API. Also, marketplaces that do not use Welcomely as a template
...
Marketplace features included in Welcomely:

- **Location search** shows the user if there are venues available in a given location.
- **The listing page** gives a detailed view about what a particular venue offers.
- **Transaction process**: Welcomely uses nightly booking.
...
- `src/marketplace.css`, `src/marketplaceIndex.css`, `src/marketplaceIndex.css` define marketplace
  ...
  Property Sets.
- `src/translations/en.json` all English translations used in Welcomely template app. (A good place
